...
LightNode::LightNode(LightType type, float power, Transform<double> transform) : Node(transform) {
  this->type = type;
  this->power = power;
}
...
std::shared_ptr<NodeUploader> strc::loadLightNode(std::shared_ptr<config::NodeCompound> root, const NodeLoader::LoadingContext & context) {

  int type = root->getNode<int>("lighttype")->getElement(0);
  float power = root->getNode<float>("power")->getElement(0);

  std::shared_ptr<strc::Node> node(new LightNode((LightNode::LightType) type, power, context.transform));
