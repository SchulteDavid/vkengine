...
void main() {
    ...
    position = transformData.transform * vec4(inPosition, 1.0);// + vec4(int(gl_InstanceIndex / 100) * 2.5, (gl_InstanceIndex % 100) * 2.5, 0, 0);
    gl_Position = ubo.proj * ubo.view * position;


    normal = vec4(normalize((transformData.transform * vec4(inNormal, 0)).xyz), 0.0);

    //normal = transformData.transform[0];
    ...
    vec3 t = normalize((transformData.transform * vec4(inTangent, 0)).xyz);
